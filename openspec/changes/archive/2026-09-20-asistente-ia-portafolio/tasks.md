...
- [x] 1.4 Controles: modelo Haiku, `max_tokens` bajo, grounding + fuera-de-alcance y degradación (verificados). Reserved concurrency **no aplicable**: la cuenta tiene el límite base de 10 ejecuciones concurrentes y AWS exige ≥10 no reservadas (`InvalidParameterValueException`). Control equivalente pre-invocación aplicado el 2026-09-20: **throttling del stage `$default` de API Gateway (`vd4c00py15`) a 2 req/s con burst 5** (`aws apigatewayv2 update-stage --default-route-settings`), verificado con `get-stages` y preflight 204.
...
- [x] 6.2 Consolidado el 2026-09-20: las 4 specs del cambio sincronizadas a `openspec/specs/` (`asistente-ia-backend`, `asistente-ia-chat-widget`, `cicd-asistente-aws`, `qr-apunta-al-sitio-aws`, con `## Purpose`, validate 24/24); el spec temprano duplicado en `mejoras-portafolio-ai-native` se retira al cerrar ese cambio (queda superseded por estos)
- [x] 6.3 Archivado el 2026-09-20 tras cerrar 1.4 (throttling en API Gateway), 6.2 (specs sincronizadas) y la decisión QR vs. canónica (QR → CloudFront; canónica pública → jalducin.github.io; documentada en la spec `qr-apunta-al-sitio-aws`)
