...
- [ ] En `index.html`, dentro de `.header-links`, reemplazar el `path` SVG de los **dos** botones `a.cv-link`
...
- [ ] Glifo candidato (un solo `<path>` para mantener el patrón actual):
...
- [ ] Sin suite automatizada; verificación manual (frontend-standards §6). N/A unitarias.

## Step 3 — Ejecutar verificación (OBLIGATORIO) — EL AGENTE EJECUTA
- [ ] Render del hero (screenshot) y confirmar que el ícono se lee como CV/résumé en dorado, con ES/EN.
- [ ] Confirmar que la descarga sigue funcionando (atributos `download` con sufijo `AAAA-MM` vía `--dump-dom`).
- [ ] Reporte en `specs/cv-icono-resume/reports/AAAA-MM-DD-step-3-verificacion.md`.

## Step 4 — Verificación manual UI (OBLIGATORIO) — EL AGENTE EJECUTA
- [ ] Revisar los tres breakpoints (992 / 768 / 480px): el ícono y las etiquetas ES/EN no se rompen.

## Step 5 — Documentación (OBLIGATORIO)
- [ ] Si cambia algo del contrato visual, sincronizar `docs/frontend-standards.md` (sección CV). (Probable: sin cambios.)

## Step 6 — Promover specs y archivar
- [ ] Merge a main, promover el delta a `openspec/specs/cv-descarga-multidioma/`, archivar el cambio, push (CI/CD).
