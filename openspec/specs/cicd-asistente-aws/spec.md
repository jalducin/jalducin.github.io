# Capability: cicd-asistente-aws

## Purpose

Despliegue automático de la Lambda del asistente vía GitHub Actions con OIDC (sin llaves estáticas), empaquetando `app.py` + `profile.txt` generado desde `llms.txt`.
